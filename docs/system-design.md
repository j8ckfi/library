...
- `library stale` (roadmap, §9) ranks nodes by `as_of`/`last_reviewed` age against the freshness budget,
...
## 8. Roadmap: CLI Ergonomics Primitives

These are specified for implementation, in priority order. Each exists to close a specific gap identified
above. Until implemented, agents use the documented manual equivalents.

| # | Command | Closes gap | Spec sketch |
| :-- | :--- | :--- | :--- |
| 1 | `library index` | Hand-maintained cheat-sheet drift | Renders `graph/INDEX.md` from `current_sota` + method redirects; `validate` warns when INDEX.md is stale vs graph state |
| 2 | `library sota --brief` / `--json` everywhere | Token economy | Tiered output per §4 |
| 3 | `library decide <task>` | Decision-shaped output | Default + alternates + `do_not_use_for` + gotchas + recipe + evidence summary + staleness flags, bounded size |
| 4 | `library stale [--max-age-days N]` | Silent rot | Ranked freshness report over `current_sota` and `last_reviewed`; exit 1 if any task exceeds budget (CI-able) |
| 5 | `library route "<natural language>"` | Routing dead-ends | Ranked candidate tasks with `scope`, `out_of_scope`, and `redirects` rendered so the agent can self-disambiguate; never returns empty without near-misses |
| 6 | `library supersede <old> <new>` | Partial supersession writes | Interactive/flag-driven transaction per §6; refuses to finish until post-conditions hold and a receipt is written |
