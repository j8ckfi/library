...
## When NOT to Use
- AppWorld coverage protocol → `method:canopy`. Async stragglers → `method:sao`. Larger-teacher distill → `method:opd`. Privileged math OPSD → `method:vista`. No teacher → `method:opsa`. Pass@1 → `method:cispo`.

## Relation to Existing SOTA
- Active plug-in on `task:outcome-only-long-horizon-agent-rl`. Mentions on `task:agentic-async-rl`, `task:student-distillation`, and `task:privileged-teacher-opsd`. Does **not** enter `current_sota`. Does **not** supersede `method:canopy`, `method:sao`, `method:opd`, `method:vista`, or `method:opsa`.
