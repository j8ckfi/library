...
## When NOT to Use
- Incremental execution-feedback playbook → `method:ace`. SWE bash loop with no bank → `method:mini-swe-agent`. Persona paging → `method:memgpt`. Dumped prompt → `method:rlm`.

## Relation to Existing SOTA
- Active on `task:agent-memory` alongside ACE. Does **not** enter `current_sota`. Does **not** replace `method:ace`.
